 ...
 * ---------------------------------------------------------------------------
 */
static int wext_freq_to_channel(int m, int e)
{
    int mhz;
...
static int channel_to_mhz(int ch, int dot11a)
{
    if (ch == 0)
    {
        return 0;
 ...
 * ---------------------------------------------------------------------------
 */
static int iwprivsdefs(struct net_device *dev, struct iw_request_info *info,
...
static int iwprivs80211ps(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
    int r = 0;
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
...
static int iwprivg80211ps(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int iwprivssmedebug(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra)
{
...
static int iwprivsapstart(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
#ifdef CSR_WIFI_FW_RELOAD
    if (interfacePriv->patchMode != 1)
    {
        unifi_error(priv, "%s : Invalid patch loaded (%d). Not starting AP\n", __FUNCTION__, interfacePriv->patchMode);
        return -EINVAL;
    }
...
static int iwprivsapconfig(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int iwprivsapstop(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
#define HW_INIT_RETRY_LIMIT  100 /* maps to 10 secs below */
static int iwprivsfwreload(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra)
{
    int r = 0;
#if defined(CSR_WIFI_FW_RELOAD) && defined(CSR_WIFI_SPLIT_PATCH)
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
    struct iw_point *mode = &wrqu->data;
    CsrWifiRouterCtrlVersions versions;
    const struct firmware *fw_entry;
    char *str = NULL;
    char fw_name[32];

    unifi_trace(priv, UDBG1, "%s\n", __FUNCTION__);

    if (priv->init_progress != UNIFI_INIT_COMPLETED)
    {
        unifi_error(priv, "%s unifi not ready, failing wext call\n", __FUNCTION__);
        return -ENODEV;
    }
    ...
    if (!mode)
    {
        unifi_error(priv, "%s : invalid parameters\n", __FUNCTION__);
        return -EINVAL;
    }

    if (interfacePriv->interfaceMode)
    {
        unifi_error(priv, "%s, FW RELOAD not allowed in this mode : %d\n", __FUNCTION__, interfacePriv->interfaceMode);
        return -EINVAL;
    }

    str = kmalloc(wrqu->data.length + 1, GFP_KERNEL);
    if (!str)
    {
        unifi_error(priv, "%s, mem alloc failed\n", __FUNCTION__);
        return -ENOMEM;
    }
    else
    {
        if (copy_from_user(str, wrqu->data.pointer, wrqu->data.length))
        {
            unifi_error(priv, "EFUALT-- \n");
            kfree(str);
            return -EFAULT;
        }
        str[wrqu->data.length] = '\0';
    }

    unifi_trace(priv, UDBG0, "%s : mode args = %s\n", __FUNCTION__, str);
    mode->flags = str[0] - 0x30;
    kfree(str);
    ...
    unifi_error(priv, "%s, request patch = %d\n", __FUNCTION__, mode->flags);
    scnprintf(fw_name, UNIFI_MAX_FW_PATH_LEN, "unifi-sdio/%s",
              (mode->flags == 0)?"staonly.xbv" : "ap.xbv");
    ...
    if (r == 0)
    {
        ...
        unifi_error(priv, "%s : Firmware file %s available\n", __FUNCTION__, fw_name);
    }
    else
    {
        unifi_error(priv, "%s : Firmware file %s not available\n", __FUNCTION__, fw_name);
        r = -ENOENT;
    ...
    /* update the patch mode here. We only maintain 0 (for sta), 1 (for AP) ,
     * since there are only two types of patches
     */
    interfacePriv->patchMode = mode->flags;

    /* Reset and download the new patch */
    r = uf_init_hw(priv);
    if (r)
    {
        unifi_error(priv, "%s : hardware init failed : %d\n", __FUNCTION__, r);
        r = -EIO;
        goto release_wakelock;
    }
     ...
     */
    CsrWifiRouterCtrlWifiOnIndSend(CSR_WIFI_SME_IFACEQUEUE, 0, CSR_RESULT_SUCCESS, versions, 1);

    while (priv->init_progress != UNIFI_INIT_COMPLETED)
    {
        static unsigned int retry = HW_INIT_RETRY_LIMIT;
        /* wait for the init to be completed */
        unifi_trace(priv, UDBG2, "%s : HW init not done .. waiting\n", __FUNCTION__);
        msleep(100);
        if (!retry--)
        {
            unifi_error(priv, "%s : HW init still not done .. give up\n", __FUNCTION__);
            return -EIO;
        }
    }
...
#endif
#endif /*defined(CSR_WIFI_FW_RELOAD) && defined(CSR_WIFI_SPLIT_PATCH) */
    return r;
}
#endif

#ifdef ANDROID_BUILD
static int iwprivsstackstart(struct net_device *dev, struct iw_request_info *info,
                             union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int iwprivsstackstop(struct net_device *dev, struct iw_request_info *info,
                            union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int iwprivsconfwapi(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra)
{
    CsrUint8 enable;
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
...
static int iwprivswpikey(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    int r = 0, i;
    CsrWifiSmeKey key;
...
static int unifi_giwname(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_siwfreq(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_giwfreq(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_siwmode(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_giwmode(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    int r = 0;
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
...
static int unifi_giwrange(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
...
static int unifi_siwap(struct net_device *dev, struct iw_request_info *info,
                       union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
    int err = 0;
    const unsigned char zero_bssid[ETH_ALEN] = {
        0x00, 0x00, 0x00,
        0x00, 0x00, 0x00
    };
...
#ifdef CSR_WIFI_FW_RELOAD
    if (interfacePriv->patchMode != 0)
    {
        unifi_error(priv, "%s : Invalid patch loaded (%d). Not connecting\n", __FUNCTION__, interfacePriv->patchMode);
        return -EINVAL;
    }
#endif
...
static int unifi_giwap(struct net_device *dev, struct iw_request_info *info,
                       union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_siwscan(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static const unsigned char* unifi_find_info_element(int id, const unsigned char *info, int len)
{
...
static int unifi_giwscan(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
 ...
 * ---------------------------------------------------------------------------
 */
static int unifi_siwessid(struct net_device *dev, struct iw_request_info *info,
...
static int unifi_giwessid(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *essid)
{
    ...
    */
    ...
    if (interfacePriv->interfaceMode == CSR_WIFI_ROUTER_CTRL_MODE_AP)
    {
        ...
        data->length = priv->ap_config.ssid.length;
        if(data->length > 0)
        {
            strncpy(essid,priv->ap_config.ssid.ssid,data->length);
            data->flags = 1;     
        } 
...
static int unifi_siwrate(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_giwrate(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
    ...
    if (interfacePriv->interfaceMode == CSR_WIFI_ROUTER_CTRL_MODE_AP ||
        interfacePriv->interfaceMode == CSR_WIFI_ROUTER_CTRL_MODE_P2PGO)
    {
        unifi_error(priv, "unifi_giwrate: not permitted in Mode %d\n",
                    interfacePriv->interfaceMode);
        return -EPERM;
    }
...
static int unifi_siwrts(struct net_device *dev, struct iw_request_info *info,
                        union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_giwrts(struct net_device *dev, struct iw_request_info *info,
                        union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
    ...
    if (interfacePriv->interfaceMode == CSR_WIFI_ROUTER_CTRL_MODE_AP ||
        interfacePriv->interfaceMode == CSR_WIFI_ROUTER_CTRL_MODE_P2PGO)
    {
        unifi_error(priv, "unifi_giwrts: not permitted in Mode %d\n",
                    interfacePriv->interfaceMode);
        return -EPERM;
    }
...
static int unifi_siwfrag(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_giwfrag(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
    ...
    if (interfacePriv->interfaceMode == CSR_WIFI_ROUTER_CTRL_MODE_AP ||
        interfacePriv->interfaceMode == CSR_WIFI_ROUTER_CTRL_MODE_P2PGO)
    {
        unifi_error(priv, "unifi_giwfrag: not permitted in Mode %d\n",
                    interfacePriv->interfaceMode);
        return -EPERM;
    }
...
static int unifi_siwencode(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_giwencode(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_siwpower(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
...
static int unifi_giwpower(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
    struct iw_param *args = &wrqu->power;
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
 ...
 * ---------------------------------------------------------------------------
 */
static int unifi_siwcommit(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra)
{
    return 0;
} /* unifi_siwcommit() */
...
static int unifi_siwmlme(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
 ...
 *
 *      Also used by CSR patched wpa_supplicant to pass in additional IEs
 *      for the probe requests for example for WPS. Use of the IE in the
 *      scan rather than another frame is indicated by the flags been set
 *      to 0x5343, This value is only set by CSR patched wpa supplicant.
 *
 *  Arguments:
 *      None.
 *
 ...
 * ---------------------------------------------------------------------------
 */
static int unifi_siwgenie(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
    ...
    /* Check for CSR special case of signal been used to set the IE for scans.
     * Magic value of flags should be unique enough to avoid any clash in future
     */
    if (0x5343 == wrqu->data.flags)
    {
        ...
        priv->scanInformationElements = kmalloc(len, GFP_KERNEL);
        if (priv->scanInformationElements == NULL)
        {
            func_exit();
            return -ENOMEM;
        }

        priv->scanInformationElementsLength = len;
        memcpy(priv->scanInformationElements, extra, len);
    }
    else
    {
...
static int unifi_giwgenie(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
 ...
 * ---------------------------------------------------------------------------
 */
static int _unifi_siwauth(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_siwauth(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
    int err = 0;
...
static int unifi_giwauth(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra)
{
 ...
 * ---------------------------------------------------------------------------
 */
static int _unifi_siwencodeext(struct net_device *dev, struct iw_request_info *info,
                               union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
static int unifi_siwencodeext(struct net_device *dev, struct iw_request_info *info,
                              union iwreq_data *wrqu, char *extra)
{
    int err = 0;
...
static int unifi_giwencodeext(struct net_device *dev, struct iw_request_info *info,
                              union iwreq_data *wrqu, char *extra)
{
    return -EOPNOTSUPP;
} /* unifi_giwencodeext() */
...
static int unifi_siwpmksa(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra)
{
    netInterface_priv_t *interfacePriv = (netInterface_priv_t *)netdev_priv(dev);
    unifi_priv_t *priv = interfacePriv->privPtr;
...
struct iw_statistics* unifi_get_wireless_stats(struct net_device *dev)
{
    ...
    if (interfacePriv->interfaceMode == CSR_WIFI_ROUTER_CTRL_MODE_AP ||
        interfacePriv->interfaceMode == CSR_WIFI_ROUTER_CTRL_MODE_P2PGO)
    {
        unifi_error(priv, "unifi_get_wireless_stats: not permitted in Mode %d\n",
                    interfacePriv->interfaceMode);
        return NULL;
    }
